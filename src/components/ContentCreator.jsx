...
import { Trophy, Award, Presentation, Zap, HelpCircle } from 'lucide-react';
import { contentCreation, socialLinks } from '../data/portfolioData';

// 1. Map text keys from your data file to actual Lucide component icons
const iconMap = {
  trophy: Trophy,
  award: Award,
...
const CreatorCard = ({ category, index }) => {
  // 2. Look up the corresponding icon or use HelpCircle as a fallback safely
  ...
  return (
    <div 
      ...
      className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-8 hover:scale-[1.02] hover:border-red-500/30 hover:shadow-[0_20px_50px_rgba(255,42,42,0.15)] transition-all duration-500 group flex flex-col justify-between h-full"
    >
      <div>
        <div className="flex justify-between items-start mb-6">
          {/* 3. Replaced emoji text container with styled vector icon rendering */}
          <div className="p-3 bg-white/5 rounded-2xl group-hover:bg-[#ff2a2a]/10 group-hover:scale-110 transition-all duration-300">
            <IconComponent className="w-8 h-8 text-white group-hover:text-[#ff2a2a] transition-colors duration-300 stroke-[1.75]" />
          </div>
      ...
      {/* Updated text to "View Post" as requested */}
  ...
  return (
    <section id="creator" className="bg-[#0a0a0a] pt-24 pb-32 px-6 md:px-12 w-full relative overflow-hidden font-sans bg-[linear-gradient(to_right,#8080800a_1px,transparent_1px),linear-gradient(to_bottom,#8080800a_1px,transparent_1px)] bg-[size:80px_80px]">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        {/* Dynamic Individual Routing Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          {contentCreation.categories.map((category, index) => (
            <a 
              key={category.title}
              href={category.link || socialLinks.linkedin} // Uses individual post link, with your profile link as fallback
              target="_blank"
        ...
        {/* Global profile link banner at bottom */}
